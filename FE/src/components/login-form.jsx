...
import { loginUser, verifyOtpUser } from "@/services/authService";
...
export function LoginForm({ className, ...props }) {
  ...
  const completeLogin = (response) => {
    ...
    const role = mapRoleById(response?.user?.roleId);
  ...
  const onSubmit = async (values) => {
    try {
      ...
      if (response?.requireOtp) {
        ...
        setOtpMessage(
          response?.message ||
            "Mã OTP đã được gửi qua email. Vui lòng nhập OTP để hoàn tất đăng nhập."
        );
        setOtpStep(true);
        otpForm.reset({ otp: "" });
                ...
                {otpStep ? (
                  <>
                    ...
                    <p className="text-muted-foreground text-balance">
                      {otpMessage || "Nhập mã OTP 6 số được gửi qua email của bạn"}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      Email: <span className="font-medium">{otpEmail || form.getValues("email")}</span>
                    </p>
                  </>
                ) : (
                  <>
